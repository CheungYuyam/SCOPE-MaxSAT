...
int*    best_soln;
int*    initial_soln;
bool    initial_soln_available = false;
bool    initial_soln_pending = false;
...
void allocate_memory()
{
	...
	initial_soln = new int [malloc_var_length];
...
bool load_initial_solution(const char *filename, string &error)
{
	ifstream input(filename);
	if (!input)
	{
		error = "cannot open initial solution file";
		return false;
	}
	...
	while (input.get(value))
	{
		if (value == '0' || value == '1')
		{
			...
			{
				error = "initial solution has too many variables";
				return false;
			}
			initial_soln[variable++] = value - '0';
		}
		else if (value != ' ' && value != '\t' && value != '\r' && value != '\n')
		{
			error = "initial solution contains a non-binary character";
			return false;
		}
	}
	if (variable != num_vars + 1)
	{
		error = "initial solution variable count does not match WCNF header";
		return false;
	}
	initial_soln_available = true;
	initial_soln_pending = true;
	return true;
}
