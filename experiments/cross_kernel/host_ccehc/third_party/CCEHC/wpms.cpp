...
string initial_solution_file;

double print_time=240.0;
double cutoff_time=296.0;
...
int parse_parameters(int argc, char *argv[])
{
	int i;
	string cur_str;
	bool inst_input = false, seed_input = false, cutoff_time_input = false;
	bool p_input = false, sp_input = false;
	...
	for(i=1; i<argc; i++)
	{
		...
		{
			i++;
			cutoff_time = strtod(argv[i], NULL);
			print_time = cutoff_time+1;
			cutoff_time_input = true;
		}
		else if(cur_str.compare("-seed-file")==0)
		{
			i++;
			initial_solution_file = argv[i];
		}
...
int main(int argc, char* argv[])
{
	...
	if(argc<7)
	{
		printf("c Usage: %s -inst <instance> -seed <seed> -t <cutoff_seconds> -p <prob> -sp <smooth_probability> [-seed-file <binary_assignment>]\n", argv[0]);
		fflush(stdout);
		return -1;
	}
	...
	{
		string seed_error;
		if (!load_initial_solution(initial_solution_file.c_str(), seed_error))
		{
			printf("c seed_file_error %s\n", seed_error.c_str());
			free_memory();
			return 2;
		}
		printf("c seeded_initial_assignment 1\n");
	}
    ...
    if(best_soln_feasible==1)
    {
		...
		if (verify_sol()==1)
		{
			printf("c final internal_verified 1\n");
			print_best_solution();
		}
		else
		{
			printf("c final internal_verified 0\n");
		}
    }
    else 
    {
