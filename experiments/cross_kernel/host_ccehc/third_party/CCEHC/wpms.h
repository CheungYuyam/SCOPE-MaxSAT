...
void init()
{
	...
	// Use the controller-provided feasible assignment for the first restart only.
	for (v = 1; v <= num_vars; v++) {
		if (initial_soln_pending)
			cur_soln[v] = initial_soln[v];
		else if(rand()%2==1) cur_soln[v] = 1;
		else cur_soln[v] = 0;
