...
bool ISDist::load_quality_seed(const char *path)
{
    ifstream input(path);
    string bits;
    if (!input || !(input >> bits))
        return false;
    if (bits == "v" && !(input >> bits))
        return false;
    if (bits.size() != static_cast<size_t>(num_vars))
        return false;

    quality_seed_solution.assign(static_cast<size_t>(num_vars) + 1, 0);
    for (int v = 1; v <= num_vars; ++v)
    {
        const char bit = bits[static_cast<size_t>(v - 1)];
        if (bit != '0' && bit != '1')
        {
            quality_seed_solution.clear();
            return false;
        }
        quality_seed_solution[v] = bit - '0';
    }
    cout << "c quality_seed loaded 1" << endl;
    return true;
}
#endif
...
#ifdef NUWLS_QUALITY_SEARCH
        else if (0 == strcmp(argv[i], "-quality_seed_file"))
        {
            i++;
            if (i >= argc || !load_quality_seed(argv[i]))
                return false;
        }
        else if (0 == strcmp(argv[i], "-quality_weight_power"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%lf", &quality_weight_power);
        }
        else if (0 == strcmp(argv[i], "-quality_phase_enabled"))
        {
            i++;
            if (i >= argc)
                return false;
            int enabled = 0;
            sscanf(argv[i], "%d", &enabled);
            quality_phase_enabled = enabled != 0;
        }
        else if (0 == strcmp(argv[i], "-quality_weight_scale"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%lf", &quality_weight_scale);
        }
        else if (0 == strcmp(argv[i], "-quality_weight_mix"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%lf", &quality_weight_mix);
        }
        else if (0 == strcmp(argv[i], "-quality_weight_anchor"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%lf", &quality_weight_anchor);
        }
        else if (0 == strcmp(argv[i], "-quality_structure_mix"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%lf", &quality_structure_mix);
        }
        else if (0 == strcmp(argv[i], "-quality_structure_mode"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%d", &quality_structure_mode);
        }
        else if (0 == strcmp(argv[i], "-quality_hard_guard"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%lf", &quality_hard_guard_factor);
        }
#endif
    }
#ifdef NUWLS_QUALITY_SEARCH
    return quality_weight_power >= 0.0 && quality_weight_power <= 1.0 &&
        quality_weight_scale > 0.0 && quality_weight_mix >= 0.0 &&
        quality_weight_mix <= 1.0 && quality_weight_anchor >= 0.0 &&
        quality_weight_anchor <= 1.0 && quality_structure_mix >= -1.0 &&
        quality_structure_mix <= 1.0 && quality_structure_mode >= 1 &&
        quality_structure_mode <= 3 && quality_hard_guard_factor >= 0.0;
#else
    return true;
#endif
}

void ISDist::settings(double allowed_time)
{
    best_known = -2;
...
#ifdef NUWLS_QUALITY_SEARCH
    quality_seed_solution.clear();
    quality_phase_enabled = true;
    quality_phase_active = false;
    quality_weight_power = 1.0;
    quality_weight_scale = 3000.0;
    quality_weight_mix = 1.0;
    quality_weight_anchor = 0.0;
    quality_structure_mix = -0.5;
    quality_structure_mode = 3;
    quality_hard_guard_factor = 1.05;
#endif
...
void ISDist::build_instance(char *filename)
{
    ...
    memset(neighbor_flag, 0, sizeof(int) * (num_vars + 1));
