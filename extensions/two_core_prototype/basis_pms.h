...
class ISDist
{
  private:
	...
	std::vector<int> quality_seed_solution;
	bool quality_phase_enabled;
	bool quality_phase_active;
	double quality_weight_power;
	double quality_weight_scale;
	double quality_weight_mix;
	double quality_weight_anchor;
	double quality_structure_mix;
	int quality_structure_mode;
	double quality_hard_guard_factor;
#endif
...
#ifdef NUWLS_QUALITY_SEARCH
	void apply_quality_weights();
	void rebuild_scores_and_goodvars();
#endif
  ...
  public:
	ISDist();
	void settings(double allowed_time);
	...
	void free_memory();
	bool verify_sol();
	bool has_feasible_solution() const { return best_soln_feasible != 0; }
#ifdef NUWLS_QUALITY_SEARCH
	bool load_quality_seed(const char *path);
#endif
